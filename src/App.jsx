...
const SAFE_AREA_BOTTOM = 'max(env(safe-area-inset-bottom), 18px)';
...
const ToastNotification = ({ show, message, type, extraClass = "", bottomOffset = 24 }) => (
  <div
    className={`fixed left-6 right-6 backdrop-blur-xl p-4 rounded-2xl shadow-2xl border flex items-center gap-3 z-[1000] transition-all duration-300 transform ${show ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0 pointer-events-none'} ${extraClass}`}
    style={{
      bottom: `calc(${bottomOffset}px + ${SAFE_AREA_BOTTOM})`,
...
const MainApp = ({ onLogout }) => {
  ...
  return (
    <div
      ...
      style={{
        paddingBottom: isScrollLocked
          ? `calc(96px + ${SAFE_AREA_BOTTOM})`
          : `calc(128px + ${SAFE_AREA_BOTTOM})`,
    ...
    >
      ...
      {/* Toast Notification */}
      <div
        className={`fixed left-6 right-6 backdrop-blur-xl p-4 rounded-2xl shadow-2xl border flex items-center gap-3 z-[200] transition-all duration-300 transform ${showToast ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0 pointer-events-none'}`}
        style={{
          bottom: `calc(96px + ${SAFE_AREA_BOTTOM})`,
          backgroundColor: '#ffffff',
          borderColor: '#f3f4f6'
        }}
      >
      ...
      <div
        className="fixed left-0 right-0 z-[150] flex items-center justify-between gap-3 px-[18px] pointer-events-none transition-all duration-300 transform translate-y-0 opacity-100"
        style={{ bottom: SAFE_AREA_BOTTOM }}
      >
